...
// 알림 읽음 처리
export const markAlertAsRead = async (alertId: string): Promise<{ item: Alert }> => {
  const response = await apiClient.put(`/user/alerts/${alertId}`);
  return response.data;
};
...
export const getUnreadAlertCount = async (): Promise<{ count: number }> => {
  const response = await apiClient.get('/user/alerts/count');
  return response.data;
};
